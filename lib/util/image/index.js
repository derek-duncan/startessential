...
function Upload(options) {
  ...
  this.options.sub_folder = options.sub_folder.toString().replace('/\//g', '.') || '';
  ...
  async.parallel({
    ...
    small: function(done) {
      ...
      gm(fileStream).size({bufferStream: true}, function(err, size) {
        ...
        if (self.options.shouldCopywrite) {
          gmObject = self._addCopywrite(this, size);
        }
        gmObject.stream(function(err, stdout, stderr) {
          gm(stdout).noProfile().resize(400).quality(75).stream(function(err, stdout) {
            self._send(stdout, options, done)
          })
        })
      })
    ...
    normal: function(done) {
      ...
      gm(fileStream).size({bufferStream: true}, function(err, size) {
        ...
        gmObject.stream(function(err, stdout, stderr) {
          gm(stdout).noProfile().resize('1200x>').quality(85).stream(function(err, stdout) {
            self._send(stdout, options, done)
          })
        })
      });
...
Upload.prototype._send = function(stream, options, callback) {
  ...
  async.waterfall([
    function(done) {
      ...
      upload.on('uploaded', function (uploadDetails) {
        return done(null, uploadDetails);
      });
...
Upload.prototype._addCopywrite = function(gmObject, size) {
  ...
  //gmObject.geometry(paddedWidth+'x'+paddedHeight+'+0+0')
