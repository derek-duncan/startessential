...
var uploader = {
  ...
  initial: function(stream, options, callback) {
    var watermark = 'http://localhost:3000/images/se-watermark.png';
    ...
    async.parallel({
      ...
      small: function(done) {
        ...
        gm(stream).size({bufferStream: true}, function(err, size) {
          ...
          //var paddedWidth = Math.floor(size.width * .8);
          //console.log(paddedWidth)
          //var paddedHeight= Math.floor(paddedWidth / watermark_ratio);
          this.composite(watermark)
          //this.out('-size', paddedWidth + 'x')
          this.gravity('Center')
