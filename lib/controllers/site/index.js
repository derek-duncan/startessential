...
function siteHomepage( request, reply ) {
  ...
  Site.findOne({ name: site_name }).populate('_user').exec(function( err, site ) {
    ...
    if (site._user.stripe.trial_expired) return reply( Boom.notFound() );
    var auth = request.auth.isAuthenticated;

    return reply.view( 'site/index', {
      title: 'Young Living Distributor Website by Start Essential',
      site: site || {},
      auth: auth
    });
  });
