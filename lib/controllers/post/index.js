...
function search(request, reply) {
  ...
  Post.find({ $text: { $search: query }}).select('title content').sort({date_created: 'desc'}).limit(limit).skip(offset).exec(function(err, posts) {
    console.log(err)
    if (err) return reply(responseHandler('fail', err));
    return reply(responseHanlder('success', posts));
  })
}
