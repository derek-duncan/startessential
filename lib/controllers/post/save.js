...
function generate(options, callback) {
  ...
  options.isPosting = options.isPosting || false;
  async.waterfall([
    function(done) {
      async.parallel([
        function(done) {
          Saved.findOne({_post: options.post_id, _user: options.uid, isOld: false}, function(err, saved) {
            if (err) return done(Boom.badImplementation('', err))
            if (saved) {
              if (options.isPosted) {
                saved.posted = true;
    ...
    function(post, done) {
      User.findOne({_id: options.uid}, function(err, user) {
        ...
        var customizeOptions = {
          ...
          color: post.options.color,
          text: user.distributor_link,
          s3Key: post.image.original.Key
        };
        var newUpload = new Upload({
          filename: post.image.original.Key,
          sub_folder: options.uid
        });
        newUpload.customizeS3Image(customizeOptions, done);
      })
    },
