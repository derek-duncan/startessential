  ...
  User.findOne({_id: uid}).select('records').exec(function(err, user) {
    ...
    if (user.hasDownload()) {
      generate(graphic_id, uid, false, function(err, saved) {
        if (err) return reply(responseHandler('err', err, err.message || 'There was an error while saving the post'));
      ...
      async.parallel([
          ...
          Saved.findOne({_post: post_id, _user: uid, isOld: false}, function(err, saved) {
            ...
            if (saved) {
              if (isPosted) {
                ...
                return done(new Error('This graphic has already been saved'))
              }
            } else {
              return done()
            }
        ...
        function(done) {
          Post.findOne({_id: post_id}, function(err, post) {
            if (err) return done(err)
            if (!post) return done(new Error('This post cannot be found'));
