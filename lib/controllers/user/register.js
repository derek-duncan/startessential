          ...
          async.parallel([
            function() {
              Email.save({ email: newUser.email, name: newUser.full_name }, function(err) {
                if (err) server.log(['error', 'email', 'save'], err);
              })
            },
            function() {
              emails.getHTML('NEW_USER', function(err, html) {
                if (err) server.log(['error', 'email', 'getHTML'], err);
                Email.send(newUser.email, html, function(err) {
                  if (err) server.log(['error', 'email', 'send'], err);
                })
              })
            }
          ], function() {
