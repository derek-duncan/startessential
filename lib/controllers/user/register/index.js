          ...
          async.parallel({
            ...
            createEmblem: function(done) {
              ...
              emblem.then(emblemDetails => {
                return done(null, emblemDetails);
              }).catch(err => {
                return done(err);
              });
            }
      ...
      function(done) {
        User.findOne( { _id: user_id } ).lean().exec(function(err, user) {
          ...
          stripe.customers.retrieve( user.stripe.id, function(err, customer) {
            ...
            var planAmount = customer.subscriptions.data.length ? customer.subscriptions.data[0].plan.amount : 1000;
            var finalAmount;
        ...
        async.parallel([
          function(done) {
            ...
            if (!customer.subscriptions.data.length) {
              stripeData.plan = 'basic';
              stripeData.trial_end = 'now';
              stripe.customers.createSubscription(user.stripe.id, stripeData, function(err, subscription) {
                return done(err, subscription.id);
              });
            } else {
              stripe.customers.update(user.stripe.id, stripeData, function(err, subscription) {
                return done(err, subscription.id);
              });
            }
          }
          ...
          user.stripe.subscription_id = subscription_id;
