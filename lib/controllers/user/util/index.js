...
const Upload = require('lib/util/image').Upload;
  ...
  createEmblem(options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      generateImage().then(imageStream => {
        ...
        return upload.send(imageStream, uploadOptions);
      }).then(uploadDetails => {
...
function test() {
  const util = new Util();
  let emblem = util.createEmblem({ name: 'Derek Duncan', memberNumber: 123481});
  emblem.then(emblemDetails => {
    console.log(emblemDetails);
  }).catch(err => {
    console.log(err);
  });
}
