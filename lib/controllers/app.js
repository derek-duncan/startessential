...
function savePostUser(request, reply) {
  ...
  _savePost(post_id, request.state.sid._id, false, function(err) {
    ...
    User.findOne({_id: request.state.sid._id}, function(err, user) {
      request.auth.session.set(user);
      return reply.redirect('/saved' + _message('Successfully saved post'));
    })
  })
}
...
function previewPostView(request, reply) {
  ...
  Saved.findOne({short_code: saved_code}).populate('_post _user').select('short_code custom_image _user.first_name _user.last_name _user.member_number _post').exec(function(err, graphic) {
    if (err) return done(Boom.badImplementation('', err))
