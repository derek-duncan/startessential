...
var UserSchema = new Schema({
  ...
  records: {
    month_renew_date: {type: Number},
    week_renew_date: {type: Number},
...
UserSchema.methods.createSite = function(done) {
  var self = this;
  var Site = mongoose.model('Site');

  Site.findOne({ _user: self._id }).exec(function(err, site) {
    if (err) return done(err);
    if (site) return done(null, site);

    var siteName = self.first_name.toLowerCase();

    Site.count({ name: siteName }).exec(function(err, count) {
...
UserSchema.methods.setTrialStatus = function(done) {
  ...
  stripe.customers.retrieveSubscription(self.stripe.id, self.stripe.subscription_id, function(err, subscription) {
    ...
    if (err && err.type !== 'StripeInvalidRequest') {
    ...
    if (!subscription) {
      ...
      trialEnd = moment.unix(subscription.trial_end);
      currentDate = moment();
      trialExpired = currentDate.isAfter(trialEnd);
    }
